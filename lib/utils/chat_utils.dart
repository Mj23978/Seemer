...
class ChatUtils {
  static final SentryClient _sentry = SentryClient(dsn: SENTRY_URL);
  ...
  static void internetLookUp() {
    try {
      InternetAddress.lookup('example.com').catchError((e) {
        ChatUtils.toast('No internet connection.');
      });
    } catch (_) {
      ChatUtils.toast('No internet connection.');
    }
      ...
      if (user[ALIAS_AVATAR] == null)
        return (user[PHOTO_URL] ?? '').isNotEmpty
            ...
            : CircleAvatar(
                ...
                child: Text(getInitials(ChatUtils.getNickname(user))),
                radius: radius,
              );
      return CircleAvatar(
  ...
  static AuthenticationType getAuthenticationType(
      bool biometricEnabled, BaseChatProvider model) {
